...
$imageLibInfo = [];
if (BulkRework::hasGD()) {
    $imageLibInfo[] = '<span class=""><i class="rex-icon fa-check"></i> GD verfügbar</span>';
}
if (BulkRework::hasImageMagick()) {
    if (class_exists('Imagick')) {
        $imageLibInfo[] = '<span class=""><i class="rex-icon fa-check"></i> ImageMagick (PHP Extension) verfügbar</span>';
    } else {
        $imageLibInfo[] = '<span class=""><i class="rex-icon fa-check"></i> ImageMagick (Binary) verfügbar</span>';
    }
}
...
$hitsPerPageForm = '<form class="uploader-bulk-rework-num-hits-per-page" action="' . rex_url::currentBackendPage() . '" method="post" style="display: inline-block; margin-right: 20px;">
    <div class="form-group form-group-sm rex-form-group-nowrap" style="display: inline-flex; align-items: center; margin: 0; gap: 10px; margin-right: 15px;">
